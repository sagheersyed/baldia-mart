...
import { X, Plus, Trash2, Calendar, Clock, RefreshCw } from 'lucide-react';
...
export default function AvailabilityModal({ doctorId, doctorName, onClose }: AvailabilityModalProps) {
  ...
  const fetchSlots = async () => {
    setLoading(true);
    try {
      const res = await fetchWithAuth(`${BASE_URL}/pharma/telemedicine/doctors/${doctorId}/availability`);
  ...
  const handleAddSlot = async () => {
    ...
    setAdding(true);
    try {
      const res = await fetchWithAuth(`${BASE_URL}/pharma/telemedicine/admin/doctors/${doctorId}/availability`, {
        method: 'POST',
  ...
  const handleDeleteSlot = async (id: string) => {
    try {
      const res = await fetchWithAuth(`${BASE_URL}/pharma/telemedicine/admin/availability/${id}`, { method: 'DELETE' });
      if (res.ok) {
        setSlots(prev => prev.filter(s => s.id !== id));
        showToast({ title: 'Slot removed', variant: 'success' });
      } else {
        const err = await parseApiError(res, 'Could not remove');
      ...
      <div className="modal-box max-w-xl" onClick={e => e.stopPropagation()}>
        <div className="modal-header bg-indigo-50 border-b border-indigo-100">
          <div>
            ...
            <p className="text-xs text-indigo-600 font-medium">Dr. {doctorName}</p>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="p-4 bg-slate-50 rounded-2xl space-y-4 border border-slate-100">
            <p className="text-[10px] font-bold text-slate-400 uppercase">Add Single Slot</p>
            ...
            <button 
              ...
              disabled={adding}
              className="w-full bg-indigo-600 text-white py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-indigo-700 transition-colors"
            >
          ...
          <div className="space-y-3">
            <p className="text-xs font-bold text-slate-800">Current Active Slots</p>
