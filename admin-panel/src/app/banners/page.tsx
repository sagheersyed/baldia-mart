...
export default function BannersPage() {
  ...
  const fetchBanners = async () => {
    try {
      setLoading(true);
      const res = await fetch('http://192.168.100.142:3000/api/v1/banners');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingBanner.id
        ? `http://192.168.100.142:3000/api/v1/banners/${editingBanner.id}`
        : 'http://192.168.100.142:3000/api/v1/banners';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetchWithAuth(`http://192.168.100.142:3000/api/v1/banners/${id}`, {
        method: 'DELETE',
      });
