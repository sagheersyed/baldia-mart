...
export default function ZonesPage() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      showToast({ title: getErrorMessage(error, 'Failed to fetch settings'), variant: 'error' });
    }
  };
  ...
  const fetchZones = async () => {
    ...
    try {
      ...
      showToast({ title: getErrorMessage(error, 'Failed to fetch zones'), variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleZoneStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      if (res.ok) {
        ...
        showToast({ title: await parseApiError(res, 'Failed to update zone status'), variant: 'error' });
      }
    } catch (error) {
      console.error('Error toggling zone:', error);
      showToast({ title: getErrorMessage(error, 'Failed to update zone status'), variant: 'error' });
    }
  };
  ...
  const toggleStoreStatus = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast({ title: `Store is now ${newStatus.toUpperCase()}`, variant: 'success' });
      } else {
        showToast({ title: await parseApiError(res, 'Failed to toggle store status'), variant: 'error' });
      }
    } catch (error) {
      console.error('Error toggling store status:', error);
      showToast({ title: getErrorMessage(error, 'Failed to toggle store status'), variant: 'error' });
    }
  };

  const updateSetting = async (key: string, value: string) => {
    try {
      const res = await fetchWithAuth(`${BASE_URL}/settings/${key}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ value }),
      });
      if (!res.ok) {
        showToast({ title: await parseApiError(res, `Failed to update ${key}`), variant: 'error' });
      } else {
        showToast({ title: 'Setting updated', variant: 'success' });
      }
    } catch (error) {
      console.error(`Error updating ${key}:`, error);
      showToast({ title: getErrorMessage(error, `Failed to update ${key}`), variant: 'error' });
    }
  };
  ...
  const handleCreateZone = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast({ title: 'Delivery zone created', variant: 'success' });
        setTimeout(() => {
        ...
        showToast({ title: await parseApiError(res, 'Failed to create zone'), variant: 'error' });
      }
    } catch (error) {
      console.error('Error creating zone:', error);
      showToast({ title: getErrorMessage(error, 'Failed to create zone'), variant: 'error' });
    } finally {
    ...
    <div className="p-8 animate-in fade-in duration-500 max-w-7xl mx-auto space-y-8">
      <header className="flex justify-between items-end">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchZones} className="p-3 bg-white border border-gray-100 rounded-xl text-gray-500 hover:text-primary-600 hover:border-primary-500-500/20 transition">
            <RefreshCcw size={18} className={loading ? 'animate-spin' : ''} />
          </button>
          <button
            onClick={() => setShowCreateModal(true)}
            className="flex items-center space-x-2 bg-primary-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-orange-600 transition shadow-lg shadow-orange-500/30"
          >
            <Plus size={20} />
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/40 border border-gray-100">
        <h2 className="text-2xl font-black text-gray-900 mb-6 flex items-center">
          <Map className="mr-3 text-primary-600" size={24} />
          Configured Regions
        </h2>
        ...
        {loading ? (
          ...
          <div className="py-20 text-center border-2 border-dashed border-gray-200 rounded-3xl">
            ...
            <button
              onClick={() => setShowCreateModal(true)}
              className="mt-6 px-8 py-3 bg-primary-600 text-white font-bold rounded-xl hover:bg-orange-600 transition shadow-md shadow-orange-500/20"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {zones.map(zone => (
              <div key={zone.id} className={`p-6 border-2 rounded-3xl relative overflow-hidden transition-all ${zone.isActive ? 'border-primary-500 shadow-md shadow-orange-500/10' : 'border-gray-200 opacity-75'}`}>
                ...
                <div className="space-y-3 relative">
                  <div className="flex justify-between items-center text-sm p-3 bg-gray-50 rounded-xl">
                    ...
                    <span className="font-black text-primary-600 text-lg">{zone.radiusKm} <span className="text-xs">km</span></span>
                  </div>
      ...
      <div className="flex space-x-6">
        ...
        <div className="flex-1 bg-gradient-to-br from-orange-50 to-red-50 p-8 rounded-[2.5rem] border border-orange-100 flex items-start space-x-4">
          <div className="p-3 bg-white rounded-2xl shadow-sm text-primary-600 shrink-0"><SettingsIcon size={24} /></div>
            ...
            <div className="mt-6 space-y-4 pt-4 border-t border-orange-100">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 bg-white border border-orange-200 rounded-lg text-orange-900 font-bold focus:outline-none focus:ring-2 focus:ring-primary-500/20"
                />
              </div>
              <div>
              ...
              <form onSubmit={handleCreateZone} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:bg-white focus:border-primary-500-500 transition font-bold"
                    value={newZone.name}
                ...
                <button
                  ...
                  className="w-full py-5 bg-gradient-to-r from-primary-600 to-orange-600 text-white rounded-[1.5rem] font-black text-lg hover:shadow-2xl hover:shadow-orange-500/30 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3"
                >
                  {isSubmitting ? (
                    <>
